...
//            }
    ...
    public int getAreaMade(int form) {
        switch (form) {
            case 20:
                return 0;
            case 40:
    ...
    public int getAreaSBD(int form) {
        switch (form) {
            case 20:
                return 2;
            case 40:
    ...
    public int checkAreaDraw(int area, int form, int j) {
...
//        Log.d("length", ((int) p1.x) + " " + height);
    ...
    public double getTH(float rate) {
        ...
        if (rate < BaseField.BORDER_WIDTH_THIN && ((double) rate) > 0.7d) {
            return 0.6d;
        }
        if (((double) rate) < 0.7d) {
            return 0.55d;
    ...
    public int getHalfRect(int form, float halfRect) {
        float rate = BaseField.BORDER_WIDTH_THIN;
        switch (form) {
            case 20:
                rate = 1.1f;
                break;
...
//            Log.d("CÂU " + j, arr[j]);
        ...
        // Gán tất cả các giá trị là -1 hết
        for (i = 0; i < rs.length; i++) {
            rs[i] = -1;
        }

        for (i = 20; i < sentence + 20; i++) {
            rs[i] = Integer.parseInt(arr[i-20]);
        }
        return rs;
    }
